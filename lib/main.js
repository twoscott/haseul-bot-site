...
$(document).ready(function () {
  $(".navbar-burger").click(function (ev) {
    ev.stopPropagation();
    $(".navbar-burger").toggleClass("is-active");
    $(".navbar-menu").toggleClass("is-active");
  });
  $(".has-dropdown").click(function (ev) {
    ev.stopPropagation();
    $(".has-dropdown").toggleClass("is-active");
  });
  $("#commands-dropdown").click(function () {
  ...
  $(".tooltip").mousemove(function (e) {
    ...
    try {
      for (var _iterator = tooltipSpans[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
        var tooltipSpan = _step.value;
...
var COMMANDS = {
  ...
  "Management": [{
    ...
    cmds: [{ name: "message", aliases: ["msg"] }, { name: "send" }],
    args: [{ name: "[channel]", examples: ["<span class=\"discord-tag\" >#general-chat</span>", '482759670637789205'] }, { name: "[content]", examples: ['Hello!'] }],
    desc: "Sends a message to channel through Haseul Bot.",
    perms: ["MANAGE_MESSAGES"]
  }, {
    cmds: [{ name: "message", aliases: ["msg"] }, { name: "edit" }],
    args: [{ name: "[message url]", examples: ["<a>https://discordapp.com/channels/482594344390623240/482594344390623244/687638163530973215</a>"] }, { name: "[new content]", examples: ['Hello! Welcome~'] }],
    desc: "Edits a message from Haseul Bot.",
    perms: ["MANAGE_MESSAGES"]
  }, {
    cmds: [{ name: "message", aliases: ["msg"] }, { name: "get" }],
    args: [{ name: "[message url]", examples: ["<a>https://discordapp.com/channels/482594344390623240/482594344390623244/687638163530973215</a>"] }],
    ...
    cmds: [{ name: "poll" }, { name: "channel" }, { name: "add" }],
    args: [{ name: "[channel]", examples: ["<span class=\"discord-tag\" >#suggestions</span>", '482596853792243712'] }],
    desc: "Adds a channel where Haseul Bot will react with a check and cross to every message set in it.",
    perms: ["MANAGE_CHANNELS"]
  }, {
    cmds: [{ name: "poll" }, { name: "channel" }, { name: "remove", aliases: ['delete'] }],
    args: [{ name: "[channel]", examples: ["<span class=\"discord-tag\" >#suggestions</span>", '482596853792243712'] }],
    desc: "Removes a channel where Haseul Bot reacts with a check and cross to every message set in it.",
    perms: ["MANAGE_CHANNELS"]
  }, {
    cmds: [{ name: "poll" }, { name: "toggle" }],
  ...
  "Moderation": [{
    cmds: [{ name: "muterole" }, { name: "set" }],
    args: [{ name: "[role name]", examples: ["Muted", "Mute", "<span class=\"discord-tag\" >@Muted</span>"] }],
    ...
    cmds: [{ name: "muterole" }, { name: "update" }],
    ...
    cmds: [{ name: "mute" }],
    args: [{ name: "[user(s)...]", examples: ["<span class=\"discord-tag\" >@Scotto</span>", "<span class=\"discord-tag\" >@User</span>", "125414437229297664"] }, { name: "(reason)", examples: ["was spamming", "was using naughty words"] }],
  ...
  "Logs": [{
    cmds: [{ name: "memberlogs", aliases: ['joinlogs', 'joins'] }, { name: "channel" }, { name: "set" }],
    args: [{ name: "[channel]", examples: ["<span class=\"discord-tag\" >#member-logs</span>", '482759389426483222'] }],
    desc: "Sets the channel for member joins and leaves to be logged in.",
    perms: ["MANAGE_CHANNELS"]
  }, {
    cmds: [{ name: "memberlogs", aliases: ['joinlogs', 'joins'] }, { name: "toggle" }],
    desc: "Toggles member logs on and off. <span class=\"has-text-grey-light\">(off by default)</span>",
    perms: ["MANAGE_GUILD"]
  }, {
    cmds: [{ name: "greeter" }, { name: "channel" }, { name: "set" }],
    args: [{ name: "[channel]", examples: ["<span class=\"discord-tag\" >#general-chat</span>", '482759670637789205'] }],
    desc: "Sets the channel for new members to be welcomed in by Haseul Bot.",
    perms: ["MANAGE_CHANNELS"]
  }, {
    cmds: [{ name: "greeter" }, { name: "message", aliases: ['msg'] }, { name: "set" }],
    args: [{ name: "[message]", examples: ["Welcome member #{memberno} - {user} to {server}!", "{default} Please read the rules in <span class=\"discord-tag\" >#rules</span>!"] }],
    ...
    cmds: [{ name: "greeter" }, { name: "toggle" }],
    desc: "Toggles welcome messages on and off. <span class=\"has-text-grey-light\">(off by default)</span>",
    perms: ["MANAGE_GUILD"]
  }, {
    cmds: [{ name: "messagelogs", aliases: ['msglogs'] }, { name: "channel" }, { name: "set" }],
    args: [{ name: "[channel]", examples: ["<span class=\"discord-tag\" >#message-logs</span>", '482759389426483222'] }],
    desc: "Sets the channel for edited and deleted messages to be logged in.",
    perms: ["MANAGE_CHANNELS"]
  }, {
    cmds: [{ name: "messagelogs", aliases: ['msglogs'] }, { name: "toggle" }],
  ...
  "Profiles": [{
    ...
    cmds: [{ name: "leaderboard" }],
    desc: "Shows the top users in the server based on XP."
  }, {
    cmds: [{ name: "leaderboard" }, { name: "global" }],
  ...
  "Last.fm": [{
    ...
    cmds: [{ name: "lastfm", aliases: ['fm', 'lf'] }, { name: "nowplaying", aliases: ['np'] }],
    args: [{ name: "(last.fm user)" }],
    ...
    cmds: [{ name: "lastfm", aliases: ['fm', 'lf'] }, { name: "yt" }],
    args: [{ name: "(last.fm user)" }],
    desc: "Shows the first result from a YouTube search for the song you or a provided user is listening to."
  }, {
    cmds: [{ name: "lastfm", aliases: ['fm', 'lf'] }, { name: "recent", aliases: ['recents'] }],
    args: [{ name: "(track count)", examples: ['100 (default is 10)'] }, { name: "(last.fm user)" }],
    ...
    cmds: [{ name: "lastfm", aliases: ['fm', 'lf'] }, { name: "toptracks", aliases: ['tt'] }],
    args: [{ name: "(time period)", examples: ['week/7day', 'month/30day', '3month/90day', '<br>6month/180day', 'year/365day', 'alltime/at/overall'] }, { name: "(track count)", examples: ['100 (default is 10)'] }],
    ...
    cmds: [{ name: "lastfm", aliases: ['fm', 'lf'] }, { name: "topalbums", aliases: ['talb', 'tal', 'tab'] }],
    args: [{ name: "(time period)", examples: ['week/7day', 'month/30day', '3month/90day', '<br>6month/180day', 'year/365day', 'alltime/at/overall'] }, { name: "(album count)", examples: ['100 (default is 10)'] }],
    ...
    cmds: [{ name: "lastfm", aliases: ['fm', 'lf'] }, { name: "topartists", aliases: ['ta'] }],
    args: [{ name: "(time period)", examples: ['week/7day', 'month/30day', '3month/90day', '<br>6month/180day', 'year/365day', 'alltime/at/overall'] }, { name: "(artist count)", examples: ['100 (default is 10)'] }],
    ...
    cmds: [{ name: "chart", aliases: ['fm', 'lf'] }],
    args: [{ name: "(grid size)", examples: ['3x3', '5x5', '10x10'] }, { name: "(time period)", examples: ['week/7day', 'month/30day', '3month/90day', '<br>6month/180day', 'year/365day', 'alltime/at/overall'] }],
    ...
    cmds: [{ name: "chart", aliases: ['fm', 'lf'] }, { name: "artists" }],
    args: [{ name: "(grid size)", examples: ['3x3', '5x5', '10x10'] }, { name: "(time period)", examples: ['week/7day', 'month/30day', '3month/90day', '<br>6month/180day', 'year/365day', 'alltime/at/overall'] }],
    ...
    cmds: [{ name: "lastfm", aliases: ['fm', 'lf'] }, { name: "profile" }],
    args: [{ name: "(last.fm user)" }],
    desc: "Shows stats from your profile or a provided user's profile."
  }],
  "YouTube": [{
    cmds: [{ name: "youtube", aliases: ['yt'] }],
    args: [{ name: "[query]" }],
    desc: "Searches YouTube for your query and returns the first 20 results."
  }],
  "Instagram - Disabled due to Instagram changes": [{
    cmds: [{ name: "instagram", aliases: ['insta'] }, { name: "notification", aliases: ['notif', 'noti'] }, { name: "add" }, { name: "<b>(DISABLED)</b>" ////
    }],
    args: [{ name: "[instagram account]", examples: ["loonatheworld", "<a>https://www.instagram.com/loonatheworld/</a>"] }, { name: "[channel]", examples: ["<span class=\"discord-tag\" >#instagram-feed</span>", '482750802377703426'] }, { name: "(mention role)", examples: ["Twitter", "notification"] }],
    ...
    cmds: [{ name: "instagram", aliases: ['insta'] }, { name: "notification", aliases: ['notif', 'noti'] }, { name: "remove", aliases: ['delete'] }, { name: "<b>(DISABLED)</b>" ////
    }],
    args: [{ name: "[instagram account]", examples: ["loonatheworld", "<a>https://www.instagram.com/loonatheworld/</a>"] }, { name: "[channel]", examples: ["<span class=\"discord-tag\" >#instagram-feed</span>", '482750802377703426'] }],
    desc: "Removes an Instagram account feed from the given channel.",
    perms: ["MANAGE_GUILD", "MANAGE_CHANNELS"]
  }, {
    cmds: [{ name: "instagram", aliases: ['insta'] }, { name: "notification", aliases: ['notif', 'noti'] }, { name: "list" }],
    desc: "Shows a list of all the Instagram feeds you have set up in the server.",
    perms: ["MANAGE_GUILD", "MANAGE_CHANNELS"]
  }],
  "Twitter": [{
    cmds: [{ name: "twitter", aliases: ['twt'] }, { name: "notification", aliases: ['notif', 'noti'] }, { name: "add" }],
    args: [{ name: "[twitter account]", examples: ["loonatheworld", "<a>https://twitter.com/loonatheworld/</a>"] }, { name: "[channel]", examples: ["<span class=\"discord-tag\" >#twitter-feed</span>", '482750802377703426'] }, { name: "(mention role)", examples: ["Twitter", "notification"] }],
    ...
    cmds: [{ name: "twitter", aliases: ['twt'] }, { name: "notification", aliases: ['notif', 'noti'] }, { name: "remove", aliases: ['delete'] }],
    args: [{ name: "[twitter account]", examples: ["loonatheworld", "<a>https://twitter.com/loonatheworld/</a>"] }, { name: "[channel]", examples: ["<span class=\"discord-tag\" >#twitter-feed</span>", '482750802377703426'] }],
    desc: "Removes a Twitter account notification from the given channel.",
    perms: ["MANAGE_GUILD", "MANAGE_CHANNELS"]
  }, {
    cmds: [{ name: "twitter", aliases: ['twt'] }, { name: "notification", aliases: ['notif', 'noti'] }, { name: "list" }],
    desc: "Shows a list of all the Twitter notifications you have in the server.",
    perms: ["MANAGE_GUILD", "MANAGE_CHANNELS"]
  }, {
    cmds: [{ name: "twitter", aliases: ['twt'] }, { name: "toggle" }, { name: "retweets", aliases: ['rts'] }],
    args: [{ name: "[twitter account]", examples: ["loonatheworld", "<a>https://twitter.com/loonatheworld/</a>"] }, { name: "[channel]", examples: ["<span class=\"discord-tag\" >#twitter-feed</span>", '482750802377703426'] }],
  ...
  "VLIVE - Currently Disabled until Haseul Bot 2.0": [{
    cmds: [{ name: "vlive" }, { name: "channel", aliases: ['channelinfosudi'] }, { name: "<b>(CURRENTLY DISABLED)</b>" ////
    }],
    args: [{ name: "[vlive channel]", examples: ["LOONA", "<a>https://channels.vlive.tv/E1F3A7/home</a>"] }],
    desc: "Shows information about a VLIVE channel."
  }, {
    cmds: [{ name: "vlive" }, { name: "notification", aliases: ['notif', 'noti'] }, { name: "add" }, { name: "<b>(CURRENTLY DISABLED)</b>" ////
    }],
    args: [{ name: "[vlive channel]", examples: ["LOONA", "<a>https://channels.vlive.tv/E1F3A7/home</a>"] }, { name: "[discord channel]", examples: ["<span class=\"discord-tag\" >#vlive-feed</span>", '482750802377703426'] }, { name: "(mention role)", examples: ["VLIVE", "notification"] }],
    ...
    cmds: [{ name: "vlive" }, { name: "notification", aliases: ['notif', 'noti'] }, { name: "remove", aliases: ['delete'] }, { name: "<b>(CURRENTLY DISABLED)</b>" ////
    }],
    args: [{ name: "[vlive channel]", examples: ["LOONA", "<a>https://channels.vlive.tv/E1F3A7/home</a>"] }, { name: "[discord channel]", examples: ["<span class=\"discord-tag\" >#vlive-feed</span>", '482750802377703426'] }],
    desc: "Removes a VLIVE channel notification from the given channel.",
    perms: ["MANAGE_GUILD", "MANAGE_CHANNELS"]
  }, {
    cmds: [{ name: "vlive" }, { name: "notification", aliases: ['notif', 'noti'] }, { name: "list" }, { name: "<b>(CURRENTLY DISABLED)</b>" ////
    }],
    desc: "Shows a list of all the VLIVE notifications you have in the server.",
    perms: ["MANAGE_GUILD", "MANAGE_CHANNELS"]
  }, {
    cmds: [{ name: "vlive" }, { name: "toggle" }, { name: "vpick" }, { name: "<b>(CURRENTLY DISABLED)</b>" ////
    }],
    args: [{ name: "[vlive channel]", examples: ["LOONA", "<a>https://channels.vlive.tv/E1F3A7/home</a>"] }, { name: "[discord channel]", examples: ["<span class=\"discord-tag\" >#vlive-feed</span>", '482750802377703426'] }],
  ...
  "Notifications": [{
    cmds: [{ name: "notification", aliases: ['notif', 'noti'] }, { name: "add" }],
    args: [{ name: "(type)", examples: ["STRICT", "NORMAL", "LENIENT"] }, { name: "[keyword]", examples: ["haseul"] }],
    ...
    cmds: [{ name: "notification", aliases: ['notif', 'noti'] }, { name: "remove", aliases: ['delete'] }],
    args: [{ name: "[keyword]", examples: ["haseul"] }],
    desc: "Removes a keyword you were notified of in the server."
  }, {
    cmds: [{ name: "notification", aliases: ['notif', 'noti'] }, { name: "list" }],
    desc: "DMs you a list of keywords for the server."
  }, {
    cmds: [{ name: "notification", aliases: ['notif', 'noti'] }, { name: "clear", aliases: ['purge'] }],
    desc: "Clears all keywords you are notified of in the server."
  }, {
    cmds: [{ name: "notification", aliases: ['notif', 'noti'] }, { name: "global" }, { name: "add" }],
    args: [{ name: "(type)", examples: ["STRICT", "NORMAL", "LENIENT"] }, { name: "[keyword]", examples: ["haseul"] }],
    ...
    cmds: [{ name: "notification", aliases: ['notif', 'noti'] }, { name: "global" }, { name: "remove", aliases: ['delete'] }],
    args: [{ name: "[keyword]", examples: ["haseul"] }],
    desc: "Removes a keyword you were notified of in servers you and Haseul Bot are both in."
  }, {
    cmds: [{ name: "notification", aliases: ['notif', 'noti'] }, { name: "global" }, { name: "list" }],
    desc: "DMs you a list of keywords you are notified of in servers you and Haseul Bot are both in."
  }, {
    cmds: [{ name: "notification", aliases: ['notif', 'noti'] }, { name: "global" }, { name: "clear", aliases: ['purge'] }],
    desc: "Clears all keywords you are notified of in servers you and Haseul Bot are both in."
  }, {
    cmds: [{ name: "notification", aliases: ['notif', 'noti'] }, { name: "ignore", aliases: ['blacklist'] }, { name: "channel", aliases: ['purge'] }],
    args: [{ name: "(channel)", examples: ["<span class=\"discord-tag\" >#general-chat</span>", '482759670637789205'] }],
    desc: "Toggles notifications for keywords in the provided channel or the channel the command is sent in."
  }, {
    cmds: [{ name: "notification", aliases: ['notif', 'noti'] }, { name: "ignore", aliases: ['blacklist'] }, { name: "server", aliases: ['purge'] }],
    desc: "Toggles notifications for keywords in the server."
  }, {
    cmds: [{ name: "notification", aliases: ['notif', 'noti'] }, { name: "dnd", aliases: ['donotdisturb'] }],
    desc: "Toggles \"do not disturb\" mode."
  }],
  "Custom Commands": [{
    cmds: [{ name: "command", aliases: ['commands', 'cmd'] }, { name: "add" }],
    args: [{ name: "[command name]", examples: ["dablivia"] }, { name: "[content]", examples: ['*dabs*', '<a>https://gfycat.com/HomelyHospitableAmericanavocet</a>'] }],
    desc: "Adds a custom command under the given command name, to respond with the provided content when used."
  }, {
    cmds: [{ name: "command", aliases: ['commands', 'cmd'] }, { name: "remove", aliases: ['delete'] }],
    args: [{ name: "[command name]", examples: ["chuuhug"] }],
    desc: "Removes a command under the given command name."
  }, {
    cmds: [{ name: "command", aliases: ['commands', 'cmd'] }, { name: "rename" }],
    args: [{ name: "[command name]", examples: ["chuuhug"] }, { name: "[new name]", examples: ["chuuhugging"] }],
    desc: "Renames a command under the given command name to the given new name."
  }, {
    cmds: [{ name: "command", aliases: ['commands', 'cmd'] }, { name: "edit" }],
    args: [{ name: "[command name]", examples: ["chuuhug"] }, { name: "[new content]", examples: ["<a>https://gfycat.com/AgitatedUnfoldedDarklingbeetle</a>"] }],
    desc: "Edits a command under the given command name's content."
  }, {
    cmds: [{ name: "command", aliases: ['commands', 'cmd'] }, { name: "list" }],
    desc: "Shows a list of all the commands in the server."
  }, {
    cmds: [{ name: "command", aliases: ['commands', 'cmd'] }, { name: "list" }, { name: "raw" }],
    desc: "Sends a JSON file with all the commands in the server."
  }, {
    cmds: [{ name: "command", aliases: ['commands', 'cmd'] }, { name: "search" }],
    args: [{ name: "[command query]", examples: ["haseul"] }],
    desc: "Shows a list of commands matching the given command query."
  }, {
    cmds: [{ name: "command", aliases: ['commands', 'cmd'] }, { name: "toggle" }],
    desc: "Toggles the ability to use custom commands on and off. <span class=\"has-text-grey-light\">(on by default)</span>"
  }],
  "Emoji": [{
    cmds: [{ name: "emoji", aliases: ['<span class=\"discord-tag\" >@Haseul Bot</span>'] }],
    args: [{ name: "[emoji]", examples: [":haseullaugh:", ':vivismirk:'] }],
    desc: "Shows a custom emoji in full size, and its file size."
  }, {
    cmds: [{ name: "emoji" }, { name: "list" }],
    desc: "Lists all the custom emojis in the server."
  }, {
    cmds: [{ name: "emoji" }, { name: "search" }],
    args: [{ name: "[query]", examples: [":haseullaugh:", ':vivismirk:'] }],
    desc: "Lists all the custom emojis matching your query."
  }],
  "Roles": [{
    cmds: [{ name: "roles" }, { name: "list" }],
    desc: "Shows a list of all the roles on the server, their member counts and their hex colour codes."
  }, {
    cmds: [{ name: "bias" }, { name: "list" }],
    desc: "Shows a list of all main and sub bias roles on the server and their member counts."
  }, {
    cmds: [{ name: "roles" }, { name: "add" }],
    args: [{ name: "[type]", examples: ["MAIN", "SUB", "OTHER"] }, { name: "[role command:name pairs...]", examples: ["olivia: 🐺 Olivia Hye, olivia hye: 🐺 Olivia Hye, hye: 🐺 Olivia Hye", "heejin: 🐰 Heejin, heekkie: 🐰 Heejin"] }],
    ...
    cmds: [{ name: "roles" }, { name: "remove", aliases: ['delete'] }],
    args: [{ name: "[type]", examples: ["MAIN", "SUB", "OTHER"] }, { name: "[role commands...]", examples: ["olivia, olivia hye, hye", "heejin, heekkie"] }],
    desc: "Removes role command pairs under a given type based on commands names.",
    perms: ["MANAGE_GUILD", "MANAGE_ROLES"]
  }, {
    cmds: [{ name: "roles" }, { name: "message", aliases: ['msg'] }, { name: "set" }],
    args: [{ name: "[message]", examples: ["__**Roles Assignment**__<br>To add a role, type +\<type\> \<role names\><br>to remove a role, type -\<type\> \<role names\><br>Types can be one of MAIN, SUB or OTHER.<br>For example, +main heejin, hyunjin, haseul"] }],
    desc: "Sets the message to be displayed in the role picker channel.",
    perms: ["MANAGE_GUILD", "MANAGE_ROLES"]
  }, {
    cmds: [{ name: "roles" }, { name: "channel" }, { name: "set" }],
    args: [{ name: "[channel]", examples: ["<span class=\"discord-tag\" >#bias-picker</span>", '482596755326763012'] }],
    desc: "Sets the role picker channel.",
    perms: ["MANAGE_GUILD", "MANAGE_ROLES"]
  }, {
    cmds: [{ name: "roles" }, { name: "toggle" }],
    desc: "Toggles the role picker. <span class=\"has-text-grey-light\">(off by default)</span>",
    perms: ["MANAGE_GUILD", "MANAGE_ROLES"]
  }, {
    cmds: [{ name: "roles" }, { name: "channel" }, { name: "update" }],
    desc: "Updates the roles channel to the current roles message and available roles.",
    perms: ["MANAGE_GUILD", "MANAGE_ROLES"]
  }, {
    cmds: [{ name: "roles" }, { name: "pairs" }, { name: "list" }],
    desc: "Lists the role commands currently paired in the server.",
    perms: ["MANAGE_GUILD", "MANAGE_ROLES"]
  }, {
    cmds: [{ name: "avarole" }],
    args: [{ name: "[type]", examples: ["MAIN", "SUB", "OTHER"] }, { name: "[commands...]", examples: ["Heejin, Hyunjin, Haseul, Yeojin, Vivi, Kim Lip, Jinsoul, Choerry, Yves, Chuu, Go Won, Olivia Hye"] }],
    desc: "Toggles commands to be shown under a type along with the roles message under Available Roles.",
    perms: ["MANAGE_GUILD", "MANAGE_ROLES"]
  }, {
    cmds: [{ name: "autorole" }, { name: "set" }],
    args: [{ name: "[role]", examples: ["New"] }],
    desc: "Sets the role to be automatically be given to new users.",
    perms: ["MANAGE_GUILD", "MANAGE_ROLES"]
  }, {
    cmds: [{ name: "autorole" }, { name: "toggle" }],
  ...
  // },
  {
    cmds: [{ name: "translate", aliases: ['tr'] }],
    args: [{ name: "[source-target]", examples: ['en-ko', 'ko-ja', 'fr-es', 'en', 'ja'] }, { name: "[text]", examples: ["I'm hungry"] }],
    ...
    cmds: [{ name: "colour", aliases: ['color'] }],
    args: [{ name: "[colour]", examples: ['#ff0000', '#03a750', '255 0 0', '34, 243, 51', 'random'] }],
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  var commandSection = document.getElementById("command-content");

  for (var category in COMMANDS) {
    if (!COMMANDS.hasOwnProperty(category)) {
      continue;
    }
    var cmdDropdown = document.getElementById("commands-dropdown");
    var commandHref = category.toLowerCase().replace(/\s+/, '-');
    ...
    var cardHeaderAnchor = document.createElement("span");
    ...
    for (var index in COMMANDS[category]) {
      var command = COMMANDS[category][index];

      if (command.cmds) {
        for (var i = 0; i < command.cmds.length; i++) {
          var _command$cmds$i = command.cmds[i],
              name = _command$cmds$i.name,
              aliases = _command$cmds$i.aliases;

          var cmdElem = createElement({
            element: "code",
          ...
          if (aliases) {
            ...
            var toolElem = document.createElement("span");
            ...
            tooltip.appendChild(createElement({
              element: "span",
              text: (i == 0 ? aliases.map(function (x) {
                return !x.startsWith('<span class=\"discord-tag\"') ? PREFIX + x : x;
              }) : aliases).join(', '),
      ...
      if (command.args) {
        for (var _i = 0; _i < command.args.length; _i++) {
          var _command$args$_i = command.args[_i],
              name = _command$args$_i.name,
              examples = _command$args$_i.examples;

          var argElem = createElement({
            element: "code",
            text: name,
            space: _i < command.args.length - 1,
            className: "has-text-info"
          });

          if (examples) {
            argElem.classList.add("tooltip");
            var _toolElem = document.createElement("span");
            _toolElem.className = "tooltip-span";

            var _tooltip = document.createElement("div");
            _tooltip.className = "card tooltip-card";

            _tooltip.appendChild(createElement({
              element: "span",
              text: examples.join(', '),
              className: "has-text-light"
            }));
            _toolElem.appendChild(_tooltip);
            argElem.appendChild(_toolElem);
          }
      ...
      if (command.perms) {
        ...
        try {
          for (var _iterator2 = command.perms[Symbol.iterator](), _step2; !(_iteratorNormalCompletion2 = (_step2 = _iterator2.next()).done); _iteratorNormalCompletion2 = true) {
